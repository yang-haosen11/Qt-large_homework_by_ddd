...
void Login::on_loginButton_clicked()
{
    ...
    if (verifyUser(username, password)) {
        ...
        GoalSetting *achievementSystem = menuWindow->achievementSystem;
        if (achievementSystem) {
            LoginStatus status = achievementSystem->handleUserLogin();

            if (status == FirstLogin) {
                QMessageBox::information(this, "首次登录", "欢迎新用户，" + username);
            } else {
